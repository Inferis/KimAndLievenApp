...
+ (NSFetchRequest *) requestAllSortedBy:(NSString *)sortTerm ascending:(BOOL)ascending withPredicate:(NSPredicate *)searchTerm inContext:(NSManagedObjectContext *)context
{
	...
	NSSortDescriptor *sortBy = [[NSSortDescriptor alloc] initWithKey:sortTerm ascending:ascending selector:@selector(caseInsensitiveCompare:)];
	[request setSortDescriptors:[NSArray arrayWithObject:sortBy]];
	[sortBy release];
...
+ (id)findOrCreateByAttribute:(NSString *)attribute withValue:(id)searchValue {
    return [self findOrCreateByAttribute:attribute 
                               withValue:searchValue 
                               inContext:[NSManagedObjectContext defaultContext]];
}

+ (id)findOrCreateByAttribute:(NSString *)attribute withValue:(id)searchValue inContext:(NSManagedObjectContext *)context {
    id foundObject = [self findFirstByAttribute:attribute withValue:searchValue inContext:context];
    if (foundObject) return foundObject;
    
    id newlyCreatedObject = [self createInContext:context];
    SEL setter = NSSelectorFromString([NSString stringWithFormat:@"%@%@%@:", @"set",[[attribute substringToIndex:1] capitalizedString],[attribute substringFromIndex:1]]);
    if ([newlyCreatedObject respondsToSelector:setter])
        [newlyCreatedObject performSelector:setter withObject:searchValue];
